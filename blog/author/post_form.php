...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === '') {
        ...
        if ($isEdit) {
            $update = $pdo->prepare('UPDATE posts SET category_id=?, title=?, slug=?, excerpt=?, content=?, image_url=?, is_published=? WHERE id=? AND user_id=?');
            $update->execute([$categoryId, $title, $slug, $excerpt, $content, $imageUrl, $isPublished, $id, $userId]);
        ...
        header('Location: /blog/author/posts.php');
        exit;
    }
}

$pageTitle = $isEdit ? 'Edit Artikel Saya' : 'Tambah Artikel Saya';
include __DIR__ . '/../includes/header.php';
?>
<div class="container" style="max-width: 900px;">
    <h1><?= $isEdit ? 'Edit Artikel Saya' : 'Tambah Artikel Saya'; ?></h1>
    <div class="form-card">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <a class="btn btn-ghost" href="/blog/author/posts.php">Kembali</a>
        </form>
    </div>
</div>
